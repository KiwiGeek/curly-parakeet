...
Public Class my_hebrew
    ...
    Public Function hebrew_leap_year(h_year As hebrew_year) As Boolean
        ' modified
        'If h_year >= 4016 Then ' post 255AD
        'Return {0, 3, 6, 8, 11, 14, 17}.Contains(CC.mod(h_year, 19))
        ' Else                    ' pre 256AD
        Return {2, 5, 7, 10, 13, 16, 18}.Contains(CC.mod(h_year, 19))
        ' End If
    End Function
    ...
    Public Function hebrew_calendar_elapsed_days(h_year As hebrew_year) As Integer              ' 7.20
        ' modified
        Dim months_elapsed As Integer
        '  If h_year >= 4016 Then              ' post 255AD
        'months_elapsed = (((h_year - 1) \ 19) * 235) + {0, 12, 24, 37, 49, 61, 74, 86, 99, 111, 123, 136, 148, 160, 173, 185, 197, 210, 222}.ToList.Item(CC.mod(h_year - 1, 19))
        ' Else                                ' pre 256AD
        months_elapsed = (((h_year - 1) \ 19) * 235) + {0, 12, 25, 37, 49, 62, 74, 87, 99, 111, 124, 136, 148, 161, 173, 185, 198, 210, 223}.ToList.Item(CC.mod(h_year - 1, 19))
        'End If
    ...
    Public Function hebrew_from_fixed([date] As fixed_date) As hebrew_date                      ' 7.28
        ' modified - just moved the approx date back 2 years, to ensure the changing in the leap pattern makes no difference
        ' probanly only needed to move one year.
